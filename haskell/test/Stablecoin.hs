...
mkInitialStorage op = let
  ...
  in (((False, BigMap ledgerMap), (BigMap operatorMap , undefined)), (mkTokenMetadata $ opTokenMetadata op, sum $ Map.elems ledgerMap ))
  where
...
specCallback c = fa2Spec $ \op ->
  ...
  in do
    ...
    pure (Nothing :: Maybe (LA.TAddress Parameter))
    -- pure (Just (LA.TAddress scContract :: LA.TAddress Parameter))
